...
Bool_t ReadOptnsFile(const TString &name, Int_t &aProj, Int_t &zProj,
                     Int_t &aTarg, Int_t &zTarg, Float_t &pnll, Float_t &bMin,
                     Float_t &bMax, Float_t &phiMin, Float_t &phiMax,
                     Int_t &evreq);
...
int main(int argc, char *argv[]) {
  ...
  if (NULL == outFile) {
    cout << "-E- main : " << outputFileName << " can not be opened!" << endl;
    return 3;
  }
  ...
  Int_t evreq;
  if (kFALSE == ReadOptnsFile(optnsFileName, aProj, zProj, aTarg, zTarg, pnll,
                              bMin, bMax, phiMin, phiMax, evreq)) {
    return 4;
  }
  ...
  Double_t sqrts =
      TMath::Sqrt(2 * kProtonMass * kProtonMass + 2 * eLab * kProtonMass);
  Double_t eCM = sqrts / 2.;
  Double_t pCM = TMath::Sqrt(eCM * eCM - kProtonMass * kProtonMass);
  Double_t pProj = +pCM;
  Double_t pTarg = -pCM;
  URun *run = new URun("venus", "", aProj, zProj, pProj, aTarg, zTarg, pTarg,
                       bMin, bMax, -1, phiMin, phiMax, 0., evreq);
  run->Print();

  // Open input file
  FILE *file = fopen(inputFileName.Data(), "r");
  if (NULL == file) {
    cout << "-E- main : " << inputFileName << " does not exist!" << endl;
    return 5;
  }
  ...
  for (Int_t iEvent = 0; iEvent < nEvents; iEvent++) {
    ...
    for (Int_t iPa = 0; iPa < nptevt; iPa++) {
      ...
      // Get PDG code
      pdgCode =
          UPIDConverter::Instance()->GetPDGCode(idptl, UPIDConverter::eWerner);
      if (pdgCode == 0) {
        cout << "-W- main : unknown VENUS particle : " << idptl << endl;
        continue;
      }

      // Add particle to the event
      event->AddParticle(iPa, pdgCode, 1, iorptl, 0, 0, 0, child, pptlx, pptly,
                         pptlz, energy, xorptlx, xorptly, xorptlz, xorptlt, 1.);
    }
