...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4) {
    cout << "Usage: " << argv[0] << " inputFileName outputFileName nEvents"
         << endl;
    return 1;
  }
  ...
  if (inFile.fail()) {
    cout << "-E- main : File " << inputFileName << " does not exist!" << endl;
    return 3;
  }
  ...
  if (NULL == outFile) {
    cout << "-E- main : File " << outputFileName << " can not be opened!"
         << endl;
    return 4;
  }
  ...
  for (Int_t iEvent = 0; iEvent < nEvents; iEvent++) {
    ...
    // Store run variables
    if (0 == iEvent) {
      run = new URun("pluto", "", 0, 0, pmxEvt, 0, 0, 0., bimEvt, bimEvt, 0, 0.,
                     6.2832, 0., nEvents);
    }
    ...
    for (Int_t iPa = 0; iPa < nptEvt; iPa++) {
      ...
      if (mode == 0) {
        ...
        inFile >> energy >> pPtlx >> pPtly >> pPtlz >> idPtl >> idSource >>
            idParent >> weight;
      } else if (mode == -2) {
        inFile >> energy >> pPtlx >> pPtly >> pPtlz >> idPtl >> idSource >>
            idParent >> indParent >> weight;
      } else if (mode == 3) {
        inFile >> energy >> pPtlx >> pPtly >> pPtlz >> xorPtlx >> xorPtly >>
            xorPtlz >> idPtl >> idSource >> idParent >> weight;
      } else if (mode == -3) {
        inFile >> energy >> pPtlx >> pPtly >> pPtlz >> xorPtlx >> xorPtly >>
            xorPtlz >> idPtl >> idSource >> idParent >> indParent >> weight;
      } else if (mode == 4) {
        inFile >> energy >> pPtlx >> pPtly >> pPtlz >> xorPtlt >> xorPtlx >>
            xorPtly >> xorPtlz >> idPtl >> idSource >> idParent >> weight;
      } else if (mode == -4) {
        inFile >> energy >> pPtlx >> pPtly >> pPtlz >> xorPtlt >> xorPtlx >>
            xorPtly >> xorPtlz >> idPtl >> idSource >> idParent >> indParent >>
            weight;
      }
      ...
      // Get particle IDs
      pdgCode =
          UPIDConverter::Instance()->GetPDGCode(idPtl, UPIDConverter::ePluto);
      if (pdgCode == 0) {
        cout << "-W- main : unknown Pluto particle : " << idPtl << endl;
        continue;
      }
      pdgParent = UPIDConverter::Instance()->GetPDGCode(idParent,
                                                        UPIDConverter::ePluto);

      // Add particle to the event
      event->AddParticle(iPa, pdgCode, 1, indParent, pdgParent, idSource, -1,
                         child, pPtlx, pPtly, pPtlz, energy, xorPtlx, xorPtly,
                         xorPtlz, xorPtlt, weight);
    }
