...
int main(int argc, char *argv[]) {
  ...
  if (argc != 5) {
    cout << "usage:   " << argv[0]
         << " AMPT-Output-File AMPT-Input-File outfile nevents\n";
  ...
  for (int n = 0; n < nevents; n++) {
    ...
    for (int i = 0; i < partNum; i++) {
      ...
      ev->AddParticle(i + 1, ID, status, parent, parent_decay, mate, decay,
                      child, px, py, pz, e, x, y, z, t, 1.);
    }
    do
      in.get(c);
    while (c != '\n');
  ...
  cout << (-pcm) << endl;
  ru = new URun(generator.data(), comment.data(), aproj, zproj, pcm, atarg,
                ztarg, -pcm, bmin, bmax, -1, 0, 0, 0, events_processed);
  ru->Write();
  fi->Write();
  fi->Close();
  return 0;
