...
int main(int argc, char *argv[]) {
  ...
  if (!inputFileB->is_open()) {
    cout << "-E- hsd2u: Cannot open " << fileNameInB << " input file!" << endl;
    return 1;
  }
  if (!inputFileM->is_open()) {
    cout << "-E- hsd2u: Cannot open " << fileNameInM << " input file!" << endl;
    return 1;
  }
  ...
  Double_t pProj = +pCM;
  Double_t pTarg = -pCM;
  URun *run = new URun("HSD", "", aProj, zProj, pProj, aTarg, zTarg, pTarg,
  ...
  for (Int_t isub = 1; isub < isubs + 1; isub++) {
    ...
    cout << "ISUB = " << isub << ",  nPip=" << (Double_t)nPip / (Double_t)num
