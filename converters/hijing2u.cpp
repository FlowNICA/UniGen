...
int main(int argc, char *argv[]) {
  ...
  if (argc != 3) {
    // cout << "usage:   " << argv[0] << " HIJING-Output-File outfile
  ...
  for (int i = 1; i <= 52; i++)
    in.ignore(777, '\n');
  // 1st Version: Energy read from hijing library output, now: SKIP LINE, for
  // loop does it anyhow
  //   in>>dust>>dust>>sqrts;
  for (int i = 1; i <= 4; i++)
    in.ignore(777, '\n');
  ...
  in >> nevents >> dust >> dust >> aproj >> atarg >> zproj >> ztarg >> dust >>
      bmin >> bmax;
  cout << "Nevents: " << nevents << " A_P: " << aproj << " A_T: " << atarg
       << " Z_P: " << zproj << " Z_T: " << ztarg << endl;

  //'dust' here holds the Reaction FRAME (CMS/LAB), might be put in proper
  //variable in future
  in >> sqrts >> dust;
  cout << sqrts << endl;
  in.ignore(777, '\n');
  ...
  for (int n = 0; n < nevents; n++) {
    ...
    for (int i = 0; i < partNum; i++) {
      ...
      ev->AddParticle(i + 1, ID, status, ichg, parent_decay, mate - 1, decay,
                      child, px, py, pz, e, x, y, z, t, weight);
    }
    do
      in.get(c);
    while (c != '\n');
  ...
  ru = new URun(generator.data(), comment.data(), aproj, zproj, pproj, atarg,
                ztarg, ptarg, bmin, bmax, -1, 0, 0, 0, events_processed);
  ru->Write();
  fi->Write();
  fi->Close();
  return 0;
