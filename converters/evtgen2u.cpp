...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4) {
    cout << "Too few parameters. Usage:" << endl
         << "evtgen2u EvtGenFile outputFile nEvents" << endl;
    return 1;
  }
  ...
  in.open(fileNameIn);
  if (!in) {
    cout << "-E- evtgen2u: Cannot open " << fileNameIn << " input file!"
         << endl;
    return 1;
  }
  ...
  Double_t pProj = 0;
  Double_t pTarg = 0;
  URun *run = new URun("EvtGen", "", aProj, zProj, pProj, aTarg, zTarg, pTarg,
                       bmin, bmax, -1, 0., TMath::TwoPi(), 0., evav);
  run->Print();
  run->Write();
  ...
  Int_t nLine = 0, pdgID = 0, nDecay = 0, nM1 = -1, nM2 = -1, nDF = -1,
        nDL = -1;
  ...
  for (Int_t iev = 0; iev < nev; iev++) {
    ...
    if (ntracks > 0) {
      ...
      for (Int_t ll = 0; ll < ntracks; ll++) {
        ...
        in >> nLine >> pdgID >> nDecay >> nM1 >> nM2 >> nDF >> nDL >> fPx >>
            fPy >> fPz >> fE >> fT >> fVx >> fVy >> fVz;
        max_nr = max(max_nr, nDF);
        max_nr = max(max_nr, nDL);
        if ((nDF == -1) && (nDL == -1)) {
          // Add particle to the event
          event->AddParticle(event->GetNpa(), pdgID, 1, 0, 0, 0, 0, child, fPx,
                             fPy, fPz, fE, fVx, fVy, fVz, fT, 1.);
        }
      } // loop over tracks
